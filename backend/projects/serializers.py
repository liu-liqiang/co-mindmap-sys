...
class ProjectSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'case_summary', 'case_amount', 'creator', 'members', 'member_count', 'node_count', 
    ...
    def get_node_count(self, obj):
        """获取项目中所有节点数（排除系统默认节点）"""
        return obj.nodes.filter(is_system_default=False).count() if hasattr(obj, 'nodes') else 0
    
    def get_my_node_count(self, obj):
        """获取当前用户在该项目中创建的节点数（排除系统默认节点）"""
        request = self.context.get('request')
        if request and request.user.is_authenticated:
            return obj.nodes.filter(
                creator=request.user, 
                is_system_default=False
            ).count() if hasattr(obj, 'nodes') else 0
        return 0
...
class ProjectCreateSerializer(serializers.ModelSerializer):
    class Meta:
        model = Project
        fields = ['name', 'case_number', 'filing_unit', 'case_summary', 'case_amount']
