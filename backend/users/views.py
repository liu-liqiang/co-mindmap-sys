...
def test_auth_status(request):
    ...
    @action(detail=False, methods=['get'], permission_classes=[IsAuthenticated])
    def search_by_real_name(self, request):
        """根据真实姓名查询用户信息"""
        real_name = request.query_params.get('real_name')
        
        if not real_name:
            return Response({'error': '请提供真实姓名参数'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            users = CustomUser.objects.filter(
                real_name=real_name,
                status='approved'
            ).order_by('department', 'police_number')
            
            if not users.exists():
                return Response({
                    'error': '此人未注册',
                    'message': '未找到此姓名的已审核用户'
                }, status=status.HTTP_404_NOT_FOUND)
...
@csrf_exempt
@require_http_methods(["POST"])
def forgot_password_verify_identity_api(request):
    """忘记密码 - 验证身份（通过警号和基本信息）"""
    try:
        data = json.loads(request.body)
        police_number = data.get('police_number')
        ...
        if not police_number or not real_name or not phone_number:
            return JsonResponse({
                'error': '警号、姓名和部门不能为空'
            }, status=400)
        ...
        try:
            ...
            return JsonResponse({
                'error': '用户信息不匹配，请检查输入的信息'
            }, status=400)
        
        # 检查用户状态
        if not user.is_approved():
            return JsonResponse({
                'error': '您的账户尚未通过审核，无法重置密码'
            }, status=403)
        ...
        # 检查是否有人脸信息
        has_face = user.is_face_registered
...
@csrf_exempt
@require_http_methods(["POST"])
def forgot_password_face_verify_api(request):
    """忘记密码 - 人脸识别验证"""
    try:
        data = json.loads(request.body)
        ...
        if not token or not face_encodings:
            return JsonResponse({
                'error': '临时令牌和人脸编码不能为空'
            }, status=400)
        ...
        try:
            payload = jwt.decode(token, settings.SECRET_KEY, algorithms=['HS256'])
            if payload.get('action') != 'forgot_password':
        ...
        if face_encodings and len(face_encodings) > 0:
            ...
            return JsonResponse({
                'message': '人脸验证成功，可以重置密码',
                'token': reset_token,
                'confidence': confidence
            })
        else:
            return JsonResponse({
                'error': '人脸识别失败：未检测到有效的人脸数据',
                'confidence': 0
            }, status=400)
...
def reset_password_api(request):
    ...
    try:
        data = json.loads(request.body)
        reset_token = data.get('token') or data.get('reset_token')  # 兼容两种字段名
        new_password = data.get('new_password')
...
def admin_reset_password_api(request):
    ...
    try:
        ...
        # 重置为初始密码
        initial_password = f"user{police_number}"
        user.set_password(initial_password)
        user.save()
        
        return JsonResponse({
            'message': f'密码已重置为初始密码：user{police_number}',
