                ...
                <div class="face-capture-section">
                    ...
                    <div class="capture-controls">
                        ...
                        <div v-else class="control-buttons">
                            <el-button type="success" @click="captureImage" :disabled="captureCount >= 3 || submitting">
                                {{ submitting ? '正在提交...' : `拍摄人脸 (${captureCount}/3)` }}
                            </el-button>

                            <el-button v-if="captureCount > 0 && !submitting" type="warning" @click="resetCapture">
                                重新拍摄
                            </el-button>

                            <div v-if="submitting" class="submitting-status">
                                <el-icon class="is-loading"><Loading /></el-icon>
                                <span>正在自动提交人脸信息...</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="tips">
                        <el-alert title="拍摄提示" type="info" show-icon :closable="false">
                            ...
                            <p>3. 需要拍摄3张不同角度的照片</p>
                            <p>4. 避免佩戴帽子、墨镜等遮挡物</p>
                            <p style="color: #E6A23C; font-weight: 500;">5. 拍摄完成后将自动提交人脸信息</p>
                        </el-alert>
                    </div>
                </div>
            </div>
...
const captureImage = async () => {
    ...
    try {
        ...
        if (captureCount.value >= 3) {
            ...
            ElMessage.info('已完成3张照片拍摄，正在自动提交人脸信息...')
