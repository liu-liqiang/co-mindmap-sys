      ...
      <template #header>
        <div class="page-header">
          <div class="header-left">
            <h1>{{ projectInfo?.name || '案件名称' }} - 思维导图</h1>
          </div>
          <div class="header-center">
            <!-- 工具栏组件放在header中间 -->
            <ToolbarVue3 @showImport="handleShowImport" @showExport="handleShowExport" />
          </div>
...
import ToolbarVue3 from './components/ToolbarVue3.vue'
...
const handleShowImport = () => {
  // 触发导入事件
  import('@/utils/bus').then(({ default: bus }) => {
    bus.emit('showImport')
  })
}
...
const initMindMap = async () => {
  ...
  try {
    ...
    const isreadonly = projectInfo.value.members.some((member: any) => 
      member.user.police_number === authStore.user?.police_number && member.permission === 'read'
    )

    console.log('当前用户是否只读:', isreadonly)
    ...
    mindMapInstance.value = new MindMap({
      ...
      beforeTextEdit: (node: any) => {
        ...
        // 只读权限只能查看，不能编辑
          ...
          node.nodeData.data._creator = authStore.user?.real_name
          node.nodeData.data._creatorPoliceNumber = authStore.user?.police_number 
          node.nodeData.data._creatorUnit = authStore.user?.department_display
          node.nodeData.data._creatorPhoneNo = authStore.user?.phone_number
        ...
        if (node.getData()._isSystemDefault || node.getData().is_system_default) {
          ...
          // 如果是当前用户创建的节点，允许编辑
          if (node.getData()._creatorPoliceNumber === authStore.user?.police_number) {
            return true // 允许编辑
          }
          // 否则，阻止编辑
      ...
      for (const item of arr) {
        try {
          ...
          if (item.action === 'create') {
            ...
            await handleNodeCreate(arr, mindMapInstance.value)
          }
          // 检查是否为更新操作
...
const handleNodeChange = async (changeItem: any) => {
  try {
    switch (changeItem.action) {
      case 'create':
        await handleNodeCreate([changeItem], mindMapInstance.value)
        break
      case 'update':
...
// 处理节点创建
const handleNodeCreate = async (arr: any[], mindMI: any) => {
  try {
    ...
    // mindMI.renderer.findNodeByUid();
    if(result&&result.success === true) {
      // 更新本地节点数据
      const newNode = mindMI.renderer.findNodeByUid(result.node_uid)
      if (newNode) {
        newNode.setData({
          ...newNode.getData(),
          _creator: result.node.creator.police_number,
          _updatedAt: result.node.updated_at,
          _synced: true
        })
        console.log('本地节点已更新服务器 ID:', result.node_id)
      }
    }
