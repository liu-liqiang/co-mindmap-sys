...
export const userAPI = {
  ...
  // 忘记密码相关API
  forgotPasswordVerifyIdentity: (data: { police_number: string; phone_number: string; real_name: string }) =>
    api.post('/users/forgot-password/verify-identity/', data),
  forgotPasswordFaceVerify: (data: { token: string; face_encodings: number[][] }) =>
    api.post('/users/forgot-password/face-verify/', data),
  resetPassword: (data: { token: string; new_password: string }) =>
    api.post('/users/reset-password/', data),
  adminResetPassword: (data: { police_number: string; admin_username?: string; admin_password?: string }) =>
    api.post('/users/admin-reset-password/', data),
  getDashboard: () =>
  ...
  // 根据真实姓名查询用户信息
  searchUsersByRealName: async (realName: string) => {
    await initializeCSRF()
    return api.get(`/users/search_by_real_name/?real_name=${encodeURIComponent(realName)}`)
  },
}
...
export const statisticsAPI = {
  ...
  getPublicStatistics: async () => {
    await initializeCSRF()
    return api.get('/statistics/public/')
  },

  // 获取单位统计数据
  getDepartmentStatistics: async () => {
    await initializeCSRF()
    return api.get('/statistics/departments/')
  }
}
